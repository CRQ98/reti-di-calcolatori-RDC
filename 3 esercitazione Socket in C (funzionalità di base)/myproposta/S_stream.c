...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if (fork() == 0)
        {
            ...
            /* PROCESS REQUEST ------------------------------------------------------------------------- */
            int nline;
            printf("Server (figlio): Waiting for request\n");
            while (read(conn_sd, &nline, sizeof(nline)) > 0)
            {
                printf("Server (figlio): Line da eliminare %d\n", nline);
                printf("Server (figlio): eseguo richiesta\n");
                int linecount = 1;
                char c;
                while (read(conn_sd, &c, 1) > 0)
                {
                    if (c == '\0')
                    {
                        write(conn_sd, &c, 1);
                ...
                printf("Server (figlio): Terminato\n");
                printf("\nServer (figlio): Waiting for a new request\n");
            } // while read nline
            printf("Server (figlio): The connection is broken, now I'm closing\n");
            close(conn_sd);
            exit(0);
        } // fork
        else
