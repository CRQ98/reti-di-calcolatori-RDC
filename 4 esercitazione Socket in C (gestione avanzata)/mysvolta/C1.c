...
TARGET:
ask DIRNAME
send COUNT request to server(dgram)
receive NFILE
*/
#include <fcntl.h>
#include <netdb.h>
...
int main(int argc, char const **argv)
{
    ...
    printf("\nInsert directory name or EOF to terminate: ");
    int nread;
    ...
    while (gets(dirname))
    {
        ...
        if (sendto(sd, &dirname, sizeof(dirname), 0, (struct sockaddr_in *)&serveraddr, len) < 0)
        {
            perror("sendto");
            printf("\nInsert directory name or EOF to terminate: ");
            continue;
        ...
        int res;
        if (recvfrom(sd, &res, sizeof(res), 0, (struct sockaddr_in *)&serveraddr, &len) < 0)
        {
            perror("recvfrom");
            printf("\nInsert directory name or EOF to terminate: ");
            continue;
